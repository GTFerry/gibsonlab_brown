...
query <- LoadFileInput(path = "azimuth-file.rds")
...
# you set in the app
cells.use <- query[["nCount_RNA", drop = TRUE]] <= 13489 &
  query[["nCount_RNA", drop = TRUE]] >= 798 &
  query[["nFeature_RNA", drop = TRUE]] <= 4010 &
  query[["nFeature_RNA", drop = TRUE]] >= 616
...
if ("percent.mt" %in% c(colnames(x = query[[]]))) {
  cells.use <- cells.use & (query[["percent.mt", drop = TRUE]] <= 48 &
